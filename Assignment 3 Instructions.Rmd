...
- If you treat missing values as having meaning, this likely means you will include them in any analysis you do on your dataset This is troublesome because those missing values are treated as having meaning though that meaning may not be appropriately defined or categorized.

- If you remove missing values, you exclude this data from your analysis. This is troublesome because could have been signficant in your analysis and conclusions
...
First let's use tidyr to convert from wide to long format.
```{r}
...
Now let's use dplyr to average our motivation values by week and by cluster.
...
I see that each cluster line has opposite changing slopes at weeks 2 and 3. For example, cluster 1 starts high, decreases, increases, than drastically decreases by week 5, while cluster 2 starts low, increases, decreases, and substantially increases by week 5.
...
K9 <- dplyr::count(K8, cluster)
print(K9)

```

Clustering by 2 is more informative for this data set. When you cluster by 3, there is a cluster where n=1. Maybe that n=1 is an outlier, but I think it is more informative as to what that n=1 is if we categorize it against other data in the dataset.

In the plot, cluster 1 where n=1 has a greater average motivation summed over the five weeks. It also experiences the greatest negative slope in the graph between weeks 4 and 5.
